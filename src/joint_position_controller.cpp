...
JointPositionController::JointPositionController(ros::NodeHandle* nh, ros::NodeHandle* pnh, std::string robot_ip, bool set_load):
    ...
    robot_.setJointImpedance({{250.0, 250.0, 150.0, 150.0, 100.0, 90.0, 50.0}});

}

void JointPositionController::initialize_control_gains(){
...
void JointPositionController::command_loop(){
    if (!gains_set_){
    ...
    while(ros::ok()){
        robot_.control([&](const franka::RobotState& robot_state, franka::Duration period) 
            ...
            return joint_position_controller_callback(robot_state, period);
        }
        );
    }
...
franka::JointPositions JointPositionController::joint_position_controller_callback(const franka::RobotState& robot_state, franka::Duration period){    
    time_ += period.toSec();
    ...
    Eigen::VectorXd::Map(&joint_positions[0], 7) = q_des_cmd_;
